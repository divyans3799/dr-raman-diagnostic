...
const App = () => {
  ...
  const tests = [
    ...
    { 
      ...
      mrp: 1800, 
      category: "Vitamin", 
      details: "Measures essential Vitamin D levels for bone density and immune regulation.",
    ...
    { 
      ...
      mrp: 950, 
      category: "Routine", 
      details: "Measures cholesterol and triglycerides for heart health assessment.",
  ...
  const packages = [
    ...
    { 
      ...
      mrp: 3500, 
      off: "63%", 
      details: "Everything in Basic + Thyroid Profile + Heart Health + Lipid Profile + LFT.",
      perks: ["Home Collection", "1 Hr Report", "Free Doctor Consult"]
    },
    { 
      ...
      mrp: 5500, 
      off: "64%", 
      details: "Gold Guard + Vitamin D + B12 + Iron Studies + Electrolytes + Bone Health.",
      perks: ["Home Collection", "1 Hr Report", "Free Doctor Consult"]
    }
  ];
  ...
  const handleBooking = (name, isExpert = false) => {
    const text = isExpert 
  ...
  return (
    <div className="min-h-screen selection:bg-primary selection:text-white">
      ...
      <div className="bg-primary text-white py-1.5 px-4 text-[10px] md:text-xs font-bold flex justify-between items-center sticky top-0 z-[130]">
        ...
        <a href={`tel:${CONTACT.phone}`} className="flex items-center gap-1 hover:text-secondary whitespace-nowrap transition-colors uppercase">
      ...
      <nav className={`transition-all duration-300 border-b sticky top-[28px] md:top-[33px] z-[120] ${isScrolled ? 'bg-white shadow-xl py-2' : 'bg-white/95 py-4'}`}>
        <div className="max-w-7xl mx-auto px-4 md:px-6 flex items-center justify-between">
          <div className="flex items-center gap-2 cursor-pointer group" onClick={() => { window.scrollTo(0,0); setIsMenuOpen(false); }}>
            <div className="bg-secondary p-1 rounded-lg shadow-md group-hover:scale-110 transition-transform">
          ...
          <div className="hidden lg:flex items-center gap-10">
            <button onClick={() => document.getElementById('packages').scrollIntoView()} className="text-[10px] font-black uppercase text-slate-400 hover:text-primary transition-all flex items-center gap-1.5 italic"><HeartPulse size={14}/> Packages</button>
            <button onClick={() => document.getElementById('tests').scrollIntoView()} className="text-[10px] font-black uppercase text-slate-400 hover:text-primary transition-all flex items-center gap-1.5 italic"><ClipboardList size={14}/> Directory</button>
            <button onClick={() => setViewRateList(true)} className="text-[10px] font-black uppercase text-slate-400 hover:text-primary transition-all flex items-center gap-1.5 italic"><FileText size={14}/> Rate List</button>
          </div>
        ...
        {isMenuOpen && (
          <div className="lg:hidden absolute top-full left-0 w-full bg-white border-b shadow-2xl animate-in fade-in slide-in-from-top-5 duration-300 z-[115]">
            <div className="flex flex-col p-6 gap-6 font-black uppercase text-[11px] tracking-widest text-slate-500">
              ...
              <button onClick={() => { setIsMenuOpen(false); document.getElementById('tests').scrollIntoView(); }} className="flex items-center gap-4 text-left"><ClipboardList size={22} className="text-secondary"/> Test Directory</button>
        ...
        <div className="max-w-2xl mx-auto relative z-10" ref={searchRef}>
          ...
          <div className="relative max-w-md mx-auto group">
            ...
            {showSuggestions && suggestions.length > 0 && (
              <div className="absolute top-full left-0 w-full bg-white border shadow-2xl rounded-b-2xl z-[150] text-left mt-1 overflow-hidden">
                {suggestions.map(s => (
                  <button 
                    key={s.id} 
                  ...
                  >
                    <span className="uppercase tracking-tighter flex items-center gap-2"><Microscope size={14} className="text-secondary"/> {s.name}</span>
      ...
      {/* 4. PACKAGES WITH DOCTOR CONSULT BRANDING */}
      <section id="packages" className="max-w-7xl mx-auto px-6 py-16">
        <h3 className="text-2xl md:text-3xl font-black text-primary uppercase italic mb-10 border-l-4 border-secondary pl-4">Health Packages</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
          {packages.map(pkg => (
            <div key={pkg.name} className="bg-primary rounded-[3rem] p-10 text-white relative overflow-hidden flex flex-col justify-between shadow-2xl">
              ...
              <div>
                ...
                <div className="bg-white/10 border border-white/20 rounded-2xl p-4 mb-8 flex items-center gap-4">
                  ...
                  <div className="text-left leading-none">
                    <p className="text-[10px] font-black uppercase text-accent tracking-widest mb-1 italic">Included Feature</p>
                    <p className="text-xs font-black uppercase tracking-tighter">Free Doctor Consultation</p>
                  </div>
                </div>
                ...
                <ul className="space-y-4 mb-12">
                  {pkg.perks.map(perk => (
                    <li key={perk} className="flex items-center gap-3 text-[11px] font-bold uppercase italic tracking-tighter">
              ...
              <div className="flex items-end justify-between border-t border-white/10 pt-8">
                <div className="text-left overflow-hidden">
                  ...
                  <p className="text-4xl md:text-5xl font-black tracking-tighter italic leading-none">₹{pkg.price}</p>
                </div>
                <button onClick={() => handleBooking(pkg.name)} className="bg-white text-primary p-5 rounded-[2.2rem] hover:bg-secondary hover:text-white transition-all shadow-xl shadow-blue-900"><ArrowRight size={28}/></button>
              </div>
            </div>
          ))}
        </div>
      ...
      {/* 5. TEST DIRECTORY DISPLAY (SEARCH RESULTS) */}
      <main id="tests" className="max-w-7xl mx-auto px-4 md:px-6 py-16 bg-white rounded-[4rem] shadow-sm mt-10">
        <div className="flex flex-col md:flex-row justify-between items-end mb-16 border-l-4 border-secondary pl-6 text-left">
          <div className="text-left">
            ...
            <p className="text-slate-400 font-black uppercase text-[8px] mt-2 italic tracking-widest">Real-time Lab Pricing</p>
          </div>
          {search && (
        ...
        {filteredTests.length > 0 ? (
              ...
              <div key={test.id} className="bg-white rounded-[2.5rem] border border-slate-100 p-8 flex flex-col justify-between hover:shadow-2xl transition-all duration-300 group text-left relative shadow-sm overflow-hidden">
                ...
                <div className="pt-6 border-t border-slate-50 flex justify-between items-end gap-3">
                  ...
                  <div className="flex flex-col gap-3 items-end shrink-0">
                    <button onClick={() => setActiveTest(test)} className="text-primary text-[10px] font-black uppercase hover:text-secondary underline underline-offset-4">Details</button>
                    <button onClick={() => handleBooking(test.name)} className="bg-primary text-white p-3 md:p-4 rounded-xl md:rounded-2xl hover:bg-secondary transition-all shadow-lg">
          ...
          <div className="py-24 text-center">
            ...
            <p className="text-slate-400 font-black uppercase tracking-widest text-sm italic">No results found for "{search}"</p>
            <button onClick={() => setSearch('')} className="text-primary font-black mt-6 underline uppercase italic text-xs">Clear Search</button>
          </div>
        )}
      </main>

      {/* 6. FOOTER WITH CORRECTED ADDRESS */}
      <footer className="bg-white pt-24 pb-12 px-6 text-left border-t border-slate-100 mt-20">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-20 pb-16 border-b border-slate-100 text-left">
          <div className="space-y-10 text-left">
            ...
            <p className="text-slate-400 text-[10px] md:text-[11px] font-bold leading-relaxed uppercase tracking-tighter italic">
              Managed by Aman Pandey. Accurate diagnostics for the holy city of Ayodhya. Trusted health reports delivered with clinical precision.
            </p>
            <div className="flex gap-5 justify-start">
               <a href={`tel:${CONTACT.phone}`} className="bg-primary text-white p-5 rounded-[2.5rem] shadow-xl transition-all"><Phone size={28}/></a>
               <button onClick={() => handleBooking('', true)} className="bg-accent text-white p-5 rounded-[2.5rem] shadow-xl transition-all"><MessageCircle size={28}/></button>
            </div>
          </div>
          ...
          <div className="space-y-12 text-left">
            ...
            <div className="flex gap-5">
              ...
              <p className="text-slate-600 font-black text-[12px] uppercase leading-tight tracking-tight italic">
                {CONTACT.address}<br/>
                <span className="text-primary bg-blue-50 px-3 py-1 rounded mt-4 inline-block italic tracking-tighter text-[10px] uppercase border border-blue-100">✓ 15km Collection Radius</span>
              </p>
            </div>
          </div>
        ...
        {/* 7. NEW RATE US COMPONENT (GOOGLE INTEGRATION) */}
        <div className="max-w-7xl mx-auto py-12 flex flex-col md:flex-row items-center justify-between gap-8 text-center md:text-left">
          <div className="space-y-2">
            <h5 className="text-primary font-black uppercase text-sm italic tracking-widest">Satisfied with our service?</h5>
            <p className="text-slate-400 font-bold uppercase text-[9px] tracking-tighter italic">Help us serve Ayodhya better by sharing your feedback.</p>
          </div>
          ...
          <a 
            ...
            className="group relative bg-white border-4 border-slate-50 p-8 md:p-10 rounded-[3rem] shadow-2xl hover:border-primary transition-all duration-500 overflow-hidden"
          >
            <div className="flex items-center gap-6 relative z-10">
              <div className="flex gap-1 text-yellow-400 star-animate">
              ...
              <div className="flex items-center gap-3">
                <span className="text-primary font-black uppercase tracking-widest text-lg md:text-xl italic">Rate Us on Google</span>
                <ThumbsUp className="text-secondary group-hover:bounce transition-all" size={28}/>
              </div>
            </div>
            <div className="absolute inset-0 bg-slate-50 translate-y-full group-hover:translate-y-0 transition-transform duration-500 -z-0 opacity-50"></div>
          </a>
        </div>

        <p className="text-center mt-12 text-[11px] font-black text-slate-300 uppercase tracking-widest italic">
          © 2026 Dr Raman Diagnostic | Aman Pandey Initiative | Ayodhya, UP
        </p>
      </footer>

      {/* 8. FIXED RATE LIST OVERLAY (NO OVERFLOW) */}
      {viewRateList && (
        <div className="fixed inset-0 z-[500] bg-white overflow-y-auto">
          <div className="p-4 md:p-12">
            <div className="max-w-4xl mx-auto border-[12px] border-primary rounded-[4rem] md:rounded-[5rem] p-8 md:p-20 relative bg-white shadow-2xl overflow-hidden">
              <button onClick={() => setViewRateList(false)} className="absolute top-8 right-8 bg-secondary text-white p-5 rounded-full shadow-2xl z-[510] hover:scale-110 transition-all"><X size={32}/></button>
              
              <div className="text-center mb-16 md:mb-24">
                <Activity className="text-secondary w-16 h-16 md:w-24 md:h-24 mx-auto mb-8" />
                <h1 className="text-primary font-black text-4xl md:text-7xl uppercase italic tracking-tighter leading-none">DR RAMAN</h1>
                <p className="text-secondary font-black text-2xl md:text-3xl tracking-[0.4em] uppercase">DIAGNOSTIC</p>
                <div className="h-2 w-48 bg-secondary mx-auto mt-10 rounded-full"></div>
                <h2 className="text-slate-800 text-xl md:text-2xl font-black uppercase tracking-tighter italic mt-10">Official Rate List — 2026</h2>
              </div>

              <div className="space-y-20 rate-table-container">
                {/* Packages Table with Fixed Width Pricing */}
                <section>
                  <div className="bg-slate-50 p-6 rounded-[2.5rem] border-l-8 border-secondary mb-10">
                    <h3 className="text-primary font-black uppercase text-sm tracking-[0.2em] italic leading-none">Health Packages (+ Free Consult)</h3>
                  </div>
                      ...
                      {packages.map(p => (
                        <tr key={p.id} className="border-b border-slate-100 text-base md:text-2xl italic uppercase tracking-tighter">
                          <td className="py-6 align-top">
                            {p.name}
                            <span className="block text-[10px] text-accent mt-1">+ FREE DOCTOR CONSULTANCY</span>
                          </td>
                          <td className="w-24 md:w-48 py-6 text-right text-primary text-3xl md:text-5xl font-black italic tracking-tighter">₹{p.price}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                ...
                <section>
                  <div className="bg-slate-50 p-6 rounded-[2.5rem] border-l-8 border-secondary mb-10">
                    <h3 className="text-primary font-black uppercase text-sm tracking-[0.2em] italic leading-none">Diagnostic Investigations</h3>
                  </div>
                  <table className="rate-table border-collapse">
                    <tbody className="text-slate-800 font-bold">
                      {tests.map(t => (
                        <tr key={t.id} className="border-b border-slate-50 text-[12px] md:text-lg italic uppercase tracking-tighter">
                          <td className="py-5 align-top">{t.name}</td>
                          <td className="w-24 md:w-48 py-5 text-right text-primary text-2xl md:text-3xl font-black italic tracking-tighter">₹{t.price}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              <div className="mt-24 md:mt-32 pt-16 border-t-4 border-slate-100 flex flex-col items-center gap-12">
                <button 
                  ...
                  className="w-full bg-accent text-white py-8 md:py-10 rounded-[3.5rem] md:rounded-[5rem] font-black uppercase tracking-[0.4em] flex items-center justify-center gap-6 hover:bg-green-700 transition-all shadow-2xl text-xs md:text-sm"
                >
                  <Headset size={32}/> Contact Our Expert
                </button>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-12 text-left w-full">
                  <div>
                    <p className="text-[11px] font-black text-secondary uppercase mb-2 italic">Support</p>
                    <p className="text-primary font-black text-3xl italic tracking-tighter">{CONTACT.display}</p>
                  </div>
                  <div>
                    <p className="text-[11px] font-black text-secondary uppercase mb-2 italic text-right">Office Address</p>
                    <p className="text-primary font-black text-[13px] uppercase tracking-tighter italic text-right leading-tight">{CONTACT.address}</p>
                  </div>
                </div>
              </div>
            </div>
      ...
      {activeTest && (
        <div className="fixed inset-0 z-[600] flex items-center justify-center p-6 bg-primary/40 backdrop-blur-md">
          <div className="bg-white rounded-[4rem] p-10 md:p-16 max-w-2xl w-full shadow-2xl relative animate-in zoom-in duration-300 text-left">
            <button onClick={() => setActiveTest(null)} className="absolute right-12 top-12 text-slate-300 hover:text-secondary"><X size={44}/></button>
            ...
            <h3 className="text-3xl md:text-4xl font-black text-primary uppercase mb-4 italic leading-tight tracking-tighter">{activeTest.name}</h3>
            ...
            <div className="bg-slate-50 p-8 rounded-[3rem] border border-slate-100 mb-12">
              <h6 className="text-[12px] font-black text-primary uppercase mb-4 tracking-widest flex items-center gap-3 italic"><Clock size={20}/> Preparation Guide</h6>
            ...
            <button 
              ...
              className="w-full bg-accent text-white py-8 md:py-10 rounded-[3.5rem] font-black uppercase tracking-[0.5em] shadow-xl flex items-center justify-center gap-4 text-[12px] md:text-[14px] transform hover:scale-[1.02] transition-all"
            >
              Book via WhatsApp <ArrowRight size={28}/>
            </button>
          </div>
        </div>
      )}
