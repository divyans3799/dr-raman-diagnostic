...
const App = () => {
  ...
  const packages = [
    ...
    { 
      ...
      off: "63%", 
      consultancy: true,
      details: "Includes 65+ Parameters: Basic Shield + Thyroid Profile + Lipid Profile + LFT.",
      perks: ["Home Collection", "1 Hr Report", "Free Doctor Consult"]
    },
    { 
  ...
  return (
    <div className="min-h-screen selection:bg-primary selection:text-white">
      ...
      <section className="bg-white py-16 px-6 border-y border-slate-100">
        <div className="max-w-7xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-8 md:gap-12 text-center md:text-left">
          <div className="space-y-4">
            ...
            <h5 className="font-black text-primary uppercase text-xs tracking-tighter italic">Expert Reports</h5>
            <p className="text-slate-500 text-[10px] leading-relaxed font-bold uppercase tracking-tighter italic">Verified by clinical experts.</p>
          </div>
          <div className="space-y-4">
            ...
            <h5 className="font-black text-primary uppercase text-xs tracking-tighter italic">1 Hr Reports</h5>
            <p className="text-slate-500 text-[10px] leading-relaxed font-bold uppercase tracking-tighter italic">Fastest delivery in Ayodhya.</p>
          </div>
          <div className="space-y-4">
            ...
            <h5 className="font-black text-primary uppercase text-xs tracking-tighter italic">Home Samples</h5>
            <p className="text-slate-500 text-[10px] leading-relaxed font-bold uppercase tracking-tighter italic">15km Radius collection support.</p>
          </div>
          <div className="space-y-4">
            ...
            <h5 className="font-black text-primary uppercase text-xs tracking-tighter italic">Reliable Care</h5>
            <p className="text-slate-500 text-[10px] leading-relaxed font-bold uppercase tracking-tighter italic">Commitment by Aman Pandey.</p>
          </div>
        </div>
      </section>
      ...
      <footer className="bg-white pt-24 pb-12 px-6 text-left border-t border-slate-100 mt-20">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-16 pb-16 border-b border-slate-100">
          <div className="space-y-8">
            ...
            <p className="text-slate-400 text-[10px] font-bold leading-relaxed uppercase tracking-tighter italic text-center md:text-left">
      ...
      {/* 8. RATE LIST MODAL (EXPERTS INTEGRATION) */}
      {viewRateList && (
        <div className="fixed inset-0 z-[500] bg-white overflow-y-auto">
          <div className="p-4 md:p-12">
            <div className="max-w-4xl mx-auto border-[12px] border-primary rounded-[4rem] p-10 md:p-20 relative bg-white shadow-2xl">
              <button onClick={() => setViewRateList(false)} className="fixed top-10 right-10 bg-secondary text-white p-6 rounded-full shadow-2xl z-[510] cursor-pointer hover:scale-110 transition-all"><X size={32}/></button>
              
              <div className="text-center mb-16 md:mb-20">
                <div className="flex justify-center items-center gap-5 mb-6">
                  <Activity className="text-secondary w-16 h-16 md:w-20 md:h-20" />
                  <div className="text-left leading-none">
                ...
                <h2 className="text-slate-800 text-3xl font-black uppercase tracking-tighter italic leading-none">Official Rate List — 2026</h2>
              ...
              <div className="space-y-16">
                <section>
                  ...
                  <table className="w-full text-left">
                    <tbody className="text-slate-800 font-black">
                      {packages.map(p => (
                        <tr key={p.id} className="border-b-2 border-slate-100 text-xl italic">
                          ...
                          <td className="py-8 px-6 text-right text-primary text-3xl md:text-5xl font-black italic tracking-tighter leading-none">₹{p.price}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                ...
                <section>
                  <div className="flex items-center gap-4 mb-10 bg-slate-50 p-6 rounded-[2.5rem]">
                    ...
                    <h3 className="text-primary font-black uppercase text-sm tracking-[0.2em] italic leading-none">Investigation Pricing</h3>
                  </div>
                  <table className="w-full text-left">
                    <tbody className="text-slate-800 font-bold">
                      {tests.map(t => (
                        <tr key={t.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors text-sm md:text-lg italic uppercase">
                          <td className="py-6 px-6 tracking-tighter leading-none">{t.name}</td>
                          <td className="py-6 px-6 text-right text-primary text-xl md:text-2xl font-black italic tracking-tighter leading-none">₹{t.price}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              <div className="mt-20 pt-16 border-t-4 border-slate-100 flex flex-col items-center">
                <button 
                  onClick={() => handleBooking('', true)}
                  className="w-full bg-accent text-white py-10 rounded-[3rem] md:rounded-[4rem] font-black uppercase tracking-[0.4em] flex items-center justify-center gap-6 hover:bg-green-700 transition-all shadow-2xl text-[14px] cursor-pointer"
                >
                  <Headset size={32}/> Contact Our Expert
                </button>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-12 text-left w-full mt-16">
                  <div>
                    <p className="text-[10px] font-black text-secondary uppercase mb-4 tracking-widest italic">Booking Assistance</p>
                    <p className="text-primary font-black text-4xl italic leading-none tracking-tighter">{CONTACT.display}</p>
                  </div>
                  <div>
                    <p className="text-[10px] font-black text-secondary uppercase mb-4 tracking-widest italic text-right">Physical Center</p>
      ...
      {activeTest && (
        <div className="fixed inset-0 z-[600] flex items-center justify-center p-6 bg-primary/40 backdrop-blur-md">
          <div className="bg-white rounded-[4rem] p-10 md:p-16 max-w-xl w-full shadow-2xl relative animate-in zoom-in duration-300 text-left">
            ...
            <div className="bg-blue-50 w-24 h-24 rounded-[2.5rem] flex items-center justify-center mb-10"><Microscope className="text-primary" size={48}/></div>
            <h3 className="text-3xl font-black text-primary uppercase mb-4 italic leading-tight tracking-tighter leading-none">{activeTest.name}</h3>
